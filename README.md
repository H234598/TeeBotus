...
Fuer nicht-user-getriggerte Proactive-Aufrufe sind Rollenkeys vorgesehen: `OPENAI_API_KEY_<INSTANCE>_PROACTIVE_PLAN`, `OPENAI_API_KEY_<INSTANCE>_PROACTIVE_DECISION` und `OPENAI_API_KEY_<INSTANCE>_PROACTIVE_WORKER`. Die aktuelle LLM-/Tool-Planung nutzt zuerst `PROACTIVE_PLAN`, danach den rollenspezifischen Services-Key wie `Depressionsbot_PROACTIVE_PLAN_SERVICES` oder den normalisierten Alias `DEPRESSIONSBOT_PROACTIVE_PLAN_SERVICES`, danach als Legacy-Fallback `OPENAI_API_KEY_<INSTANCE>_PROACTIVE` und zuletzt ausschliesslich fuer die Depressionsbot-Instanz `Depressionsbot_BACKGROUND_SERVICES`. Normale instanzweite oder globale OpenAI-Keys werden fuer `proactive`/`background` nicht als Fallback genutzt; Telegram-/Signal-/Matrix-Userantworten bleiben davon getrennt.
