...
CITY_CHANGE_PATTERNS = (
    re.compile(
        ...
        r"(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben))\s+(?:im|in\s+dem|in\s+der)\s+"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)(?:er|s)\s+stadtgebiet\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:(?:mein(?:e)?|unser(?:e)?)?\s*(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|"
...
CITY_PATTERNS = (
    ...
    re.compile(
        ...
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)(?<!s)s\s+(?:nähe|naehe|umgebung|umland)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:mein(?:e)?|unser(?:e)?)?\s*(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz)\s+"
