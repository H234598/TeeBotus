...
_RESIDENCE_DURATION = (
    ...
    r"(?:meiner\s+)?(?:kindheit|jugend|geburt)|(?:dem|meinem)\s+(?:studium|umzug|einzug)|"
    r"(?:dem\s+)?(?:beginn|ende)\s+(?:meines|des)\s+studiums|(?:dem|meinem)\s+ersten\s+tag|"
...
CITY_CHANGE_PATTERNS = (
    ...
    re.compile(
        ...
        r"(?:die|der|das)\s+(?:stadt|ort)\s*,?\s*"
        r"(?:in\s+der|an\s+dem|wo)\s+(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\b",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:der|die|das)\s+(?:ort|stadt)\s*,?\s*"
...
CITY_PATTERNS = (
    ...
    re.compile(
        ...
        r"(?:ortschaft|gemeinde|kommune|metropole|hauptstadt|hansestadt|hafenstadt|"
    ...
    re.compile(
        r"\b(?:ich\s+)?(?:wohne|wohnen|lebe|leben)\s+"
        r"(?!(?:ich|wir|tue|tun|mal|teils|abwechselnd|aber|doch|jedoch|zwischen|irgendwo|mit|auf|aus|nach|für|fuer|ab|seit|"
