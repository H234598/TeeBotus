...
_CURRENT_RESIDENCE_FORWARD_QUALIFIER = (
    r"(?:aktuell\w*|offiziell\w*|privat\w*|gemeldet\w*|amtlich\w*|neu\w*|"
    r"jetzig\w*|derzeitig\w*|momentan\w*|gegenwärtig\w*|gegenwaertig\w*|"
    r"tatsächlich\w*|tatsaechlich\w*|jetzt|nun|dauerhaft\w*|permanent\w*|"
    r"fest\w*|hauptsächlich\w*|hauptsaechlich\w*|ständig\w*|staendig\w*|"
    r"stabil\w*|inzwischen|mittlerweile|weiterhin|nach\s+wie\s+vor|"
    r"seit\s+(?:heute|gestern|vorgestern)|ab\s+(?:sofort|jetzt))"
)
...
_CITY_CHANGE_LABELLED_OLD_CURRENT = re.compile(
    ...
    r"(?:früher|frueher|ehemals|damals|vorher|zuvor)\s+"
    r"(?:(?:in|bei)\s+)?[^,.;!?]{1,80}?\s*[,;]\s*"
...
_CITY_CHANGE_OLD_RESIDENCE_LABEL_CURRENT_CITY = re.compile(
    r"\b(?:mein(?:e)?|unser(?:e)?)\s+(?:alt\w*|ehemalig\w*|früh\w*|frueh\w*)\s+"
    r"(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|wohnadresse|wohnanschrift|adresse|anschrift|"
    r"zuhause|zu\s+hause|daheim)\s+(?:war|ist)\s+(?:(?:in|bei)\s+)?"
    r"(?P<old_city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)\s*[,;]\s*"
...
_CITY_CHANGE_OLD_RESIDENCE_LABEL_BARE_CURRENT_CITY = re.compile(
    r"\b(?:mein(?:e)?|unser(?:e)?)\s+(?:alt\w*|ehemalig\w*|früh\w*|frueh\w*)\s+"
    r"(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|wohnadresse|wohnanschrift|adresse|anschrift|"
...
_DIRECT_RESIDENCE_BEFORE_CURRENT_FORWARD_LABEL = (
    re.compile(
        r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+(?:in|bei)\s+"
        rf"(?P<first>{_CITY_CHANGE_CITY_FRAGMENT})\s*[.!?]\s*"
        rf"(?:mein(?:e)?|unser(?:e)?)\s+{_CURRENT_RESIDENCE_FORWARD_QUALIFIER}\s+"
        r"(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|lebensmittelpunkt|"
        r"wohnadresse|wohnanschrift|privatadresse|privatanschrift|adresse|anschrift|"
        r"meldeadresse|meldeanschrift|meldesitz|zuhause|zu\s+hause|daheim)\s+"
        r"(?:ist|lautet|liegt|befindet\s+sich|bleibt)\s+(?:(?:in|bei)\s+)?"
        rf"(?P<second>{_CITY_CHANGE_CITY_FRAGMENT})(?=\s*[.!?]\s*$)",
        re.IGNORECASE,
    ),
    re.compile(
        r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+(?:in|bei)\s+"
        ...
        r"wohnadresse|wohnanschrift|privatadresse|privatanschrift|adresse|anschrift|"
        r"meldeadresse|meldeanschrift|meldesitz|zuhause|zu\s+hause|daheim)\s+"
        rf"(?:ist|lautet|liegt|befindet\s+sich|bleibt)\s+{_CURRENT_RESIDENCE_FORWARD_QUALIFIER}\s+"
        rf"(?:(?:in|bei)\s+)?(?P<second>{_CITY_CHANGE_CITY_FRAGMENT})(?=\s*[.!?]\s*$)",
        re.IGNORECASE,
    ),
)
_DIRECT_RESIDENCE_BEFORE_TEMPORARY_FORWARD_LABEL = (
    re.compile(
        r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+(?:in|bei)\s+"
        rf"(?P<first>{_CITY_CHANGE_CITY_FRAGMENT})\s*[.!?]\s*"
        r"(?:mein(?:e)?|unser(?:e)?)\s+"
    ...
    re.compile(
        ...
        rf"(?:{_TEMPORARY_RESIDENCE_ADJECTIVE})\s+(?:(?:in|bei)\s+)?"
        rf"(?P<second>{_CITY_CHANGE_CITY_FRAGMENT})(?=\s*[.!?]\s*$)",
        re.IGNORECASE,
    ),
)
...
def _direct_residence_before_current_forward_label(source: str) -> str:
    for pattern in _DIRECT_RESIDENCE_BEFORE_CURRENT_FORWARD_LABEL:
        match = pattern.search(source)
        if not match:
            continue
        first = _clean_city(match.group("first"))
...
def _direct_residence_before_temporary_forward_label(source: str) -> str:
    for pattern in _DIRECT_RESIDENCE_BEFORE_TEMPORARY_FORWARD_LABEL:
        match = pattern.search(source)
        if not match:
            continue
        first = _clean_city(match.group("first"))
...
_INVERTED_RELATIVE_RESIDENCE_CITY = re.compile(
    r"(?:"
    r"\bwo\s+(?:ich|wir)\s+(?:(?:offiziell|amtlich|polizeilich)\s+)?"
    r"(?:gemeldet|registriert|wohnhaft|ansässig|ansaessig)\s+"
    r"(?:bin|sind)\s*,?\s*(?:da|dort|hier)\s+"
    r"(?:wohne|wohnen|lebe|leben)\s+(?:ich|wir)"
    r"|"
    r"\bwo\s+(?:ich|wir)\s+(?:meinen|unseren)\s+"
    r"(?:(?:offiziell|amtlich|polizeilich)\w*\s+)?"
    r"(?:wohnort|wohnsitz|hauptwohnsitz|lebensmittelpunkt)\s+(?:habe|haben)\s*,?\s*(?:da|dort|hier)\s+"
    r"(?:wohne|wohnen|lebe|leben)\s+(?:ich|wir)"
    r"|"
    r"\bwo\s+(?:mein|unser)\s+"
    r"(?:(?:offiziell|amtlich|polizeilich)\w*\s+)?"
    r"(?:wohnort|wohnsitz|hauptwohnsitz|lebensmittelpunkt)\s+"
    r"(?:ist|liegt|befindet\s+sich)\s*,?\s*(?:da|dort|hier)\s+"
    r"(?:wohne|wohnen|lebe|leben)\s+(?:ich|wir)"
    r"|"
    r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+(?:dort|da|hier)\s*,?\s*wo\s+"
    r"(?:ich|wir)\s+(?:(?:offiziell|amtlich|polizeilich)\s+)?"
    r"(?:gemeldet|registriert|wohnhaft|ansässig|ansaessig)\s+(?:bin|sind)"
    r"|"
    r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+(?:dort|da|hier)\s*,?\s*wo\s+"
...
_CITY_BEFORE_RELATIVE_RESIDENCE_LABEL = re.compile(
    ...
    r"(?:meinen|meine|meinem|unseren|unsere|unserem)\s+"
    rf"(?:(?:{_RESIDENCE_LABEL_CURRENT_QUALIFIER})\s+)?"
    r"(?:wohnort|wohnsitz|hauptwohnsitz|lebensmittelpunkt|zuhause|zu\s+hause|wohnung|"
    r"wohnadresse|wohnanschrift|meldeadresse|meldeanschrift|meldesitz|adresse|anschrift)\s+(?:habe|haben)|"
    r"(?:mein|meine|meinem|unser|unsere|unserem)\s+"
    rf"(?:(?:{_RESIDENCE_LABEL_CURRENT_QUALIFIER})\s+)?"
    r"(?:wohnort|wohnsitz|hauptwohnsitz|lebensmittelpunkt|zuhause|zu\s+hause|wohnung|"
    r"wohnadresse|wohnanschrift|meldeadresse|meldeanschrift|meldesitz|adresse|anschrift)\s+"
    r"(?:ist|liegt|befindet\s+sich)|"
    r"sich\s+(?:mein|meine|meinem|unser|unsere|unserem)\s+"
    rf"(?:(?:{_RESIDENCE_LABEL_CURRENT_QUALIFIER})\s+)?"
    r"(?:wohnort|wohnsitz|hauptwohnsitz|lebensmittelpunkt|zuhause|zu\s+hause|wohnung|"
    r"wohnadresse|wohnanschrift|meldeadresse|meldeanschrift|meldesitz|adresse|anschrift)\s+befindet"
    r")",
    re.IGNORECASE,
)
...
_CITY_CHANGE_LABELLED_CURRENT_NEW_RESIDENCE = re.compile(
    ...
    r"(?:[:=,]\s*|\s+)"
    r"(?P<old_city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)\s*[,;]\s*"
    r"(?:neu\w*|aktuell\w*|jetzig\w*|gegenwärtig\w*|gegenwaertig\w*)\s+"
    r"(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|lebensmittelpunkt)\s*:?[ \t]*"
    r"(?!(?:arbeite|arbeitest|arbeiten|studier|lern|schlaf|pendl|reis|besuch|"
    r"übernacht|uebernacht|fahr|geh|komm|mach)\w*\b)"
    r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)(?=\s*(?:[.!?;,]|$))",
...
_CITY_CHANGE_LABELLED_OLD_NEW_ADDRESS = re.compile(
    r"\b(?:(?:mein(?:e)?|unser(?:e)?)\s+)?"
    r"(?:alt\w*|ehemalig\w*|früh\w*|frueh\w*)\s+"
    r"(?:wohnadresse|wohnanschrift|adresse|anschrift)\s*:?[ \t]*"
    r"(?P<old_city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)\s*[,;]\s*"
...
_CITY_CHANGE_LABELLED_SHORT_CURRENT = re.compile(
    r"\b(?:(?:mein(?:e)?|unser(?:e)?)\s+)?"
    r"(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|lebensmittelpunkt)\s*[:=]\s*"
    r"(?P<old_city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)\s*[.!?;]\s*"
    r"(?:jetzt|nun|aktuell|derzeit|inzwischen|mittlerweile|heute)\s*[:=]\s*"
    r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)(?=\s*(?:[.!?;,]|$))",
    re.IGNORECASE,
)
...
CITY_CHANGE_PATTERNS = (
    ...
    re.compile(
        ...
        r"(?:mein(?:e)?|unser(?:e)?)\s+(?:neu\w*|aktuell\w*|jetzig\w*)\s+"
        r"(?:(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|wohnadresse|wohnanschrift|adresse|anschrift|"
        r"zuhause|zu\s+hause|daheim)\s+)?(?:ist|liegt|befindet\s+sich)\s+(?:(?:in|bei)\s+)?"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80})",
        re.IGNORECASE,
    ),
    ...
    re.compile(
        r"\b(?:meine|unsere)\s+(?:alte|ehemalige|frühere|fruehere)\s+"
        r"(?:wohnadresse|wohnanschrift|adresse|anschrift)\s+war\s+[^,.;!?]{1,80},\s*"
        r"(?:die\s+)?(?:neue|aktuelle|jetzige)\s+"
        r"(?:(?:wohnadresse|wohnanschrift|adresse|anschrift)\s+)?"
        r"(?:ist|liegt|befindet\s+sich)\s+(?:in|bei)?\s*"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80})",
        re.IGNORECASE,
    ...
    re.compile(
        r"\b(?:früher|frueher|ehemals|damals)\s+(?:war|bin|ist|sind)\s+(?:ich|wir)\s+"
        r"(?:in|bei)\s+[^,.;!?]{1,80}\s+(?:wohnhaft|ansässig|ansaessig)\s*[,;]\s*"
        r"(?:jetzt|heute|nun|aktuell|derzeit|inzwischen|mittlerweile)\s+(?:in|bei)\s+"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80})",
        re.IGNORECASE,
    ),
    ...
    re.compile(
        r"\b(?:ich|wir)\s+war(?:en)?\s+(?:(?:früher|frueher|vorher|damals|ehemals|zuvor)\s+)?"
        r"(?:in|bei)\s+[^,.;!?]{1,80}(?:,|;|[-–—])\s*(?:aber\s+)?"
        r"(?:jetzt|heute|nun|aktuell|derzeit|inzwischen|mittlerweile)\s+"
        r"(?:(?:in|bei)\s+)?"
        r"(?!(?:arbeite|arbeitest|arbeiten|studier|lern|schlaf|pendl|reis|besuch|"
    ...
    re.compile(
        r"\b(?:ich|wir)\s+(?:wohne|lebe)\s+(?:nicht\s+(?:mehr|l(?:aenger|änger))?|nicht)\s+"
        r"(?:in|bei)\s+[^,.;!?]{1,80}?\s*(?:,|;|[-–—])\s*"
        r"(?:sondern|aber|doch|jedoch)\s+(?:wohne|lebe)\s+"
    ...
    re.compile(
        ...
        r"(?:in\s+der\s+(?:nähe|naehe|umgebung|gegend)\s+von|nahe|unweit\s+von|"
        r"im\s+umland(?:\s+von)?|in|bei)\s+(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80})",
        re.IGNORECASE,
    ),
    re.compile(
    ...
    re.compile(
        r"\b(?:ich\s+)?(?:wohne|lebe|wohnen|leben)\s+(?:aber|doch|jedoch)\s+"
        r"(?:(?:jetzt|heute|nun|aktuell|derzeit|inzwischen|mittlerweile)\s+)?(?:in|bei)\s+"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80})",
        re.IGNORECASE,
    ),
    ...
    re.compile(
        r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+"
        r"(?:in\s+der\s+(?:naehe|n(?:ä|ae)he|umgebung|gegend)\s+von|nahe|unweit\s+von|"
        r"im\s+umland(?:\s+von)?|im\s+(?:[A-ZÄÖÜ][\wÄÖÜäöüß'-]+\s+)?raum(?:\s+von)?)\s+"
        r"[^,.;!?]{1,80},\s*"
        r"(?:genauer(?:\s+gesagt)?|konkret|nämlich|naemlich|und\s+zwar|"
    ...
    re.compile(
        r"\b(?:ich|wir)\s+(?:wohne|wohnen|lebe|leben)\s+"
        r"im\s+(?:[A-ZÄÖÜ][\wÄÖÜäöüß'-]+\s+)?raum\s*,\s*"
        r"(?:genauer(?:\s+gesagt)?|konkret|nämlich|naemlich|und\s+zwar|"
        r"besser\s+gesagt|sprich)\s+(?:in|bei)\s+"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)"
        r"(?=\s*(?:[.!?;,]|$))",
    ...
    re.compile(
        r"\b(?:mein(?:e)?|unser(?:e)?)?\s*"
        r"(?:wohnort|wohnsitz|wohnstadt)\s*:\s*"
        r"[^,.;!?]{1,80},\s*"
        r"(?:genauer(?:\s+gesagt)?|konkret|nämlich|naemlich|und\s+zwar|"
        r"besser\s+gesagt|sprich)\s*:?[\s]*(?:(?:in|bei)\s+)?"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)"
        r"(?=\s*(?:[.!?;,]|$))",
        re.IGNORECASE,
    ),
...
CITY_PATTERNS = (
    ...
    re.compile(
        ...
        r"(?:neu\w*|jetzig\w*|aktuell\w*|derzeitig\w*|gegenwärtig\w*|gegenwaertig\w*)\s+"
        r"(?:wohnort|wohnsitz|wohnstadt|hauptwohnsitz|wohnadresse|wohnanschrift|adresse|anschrift|"
        r"zuhause|zu\s+hause|daheim)\s*(?::|=|,)?\s*"
        r"(?P<city>[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?)(?=\s*(?:[.!?;,]|$))",
        re.IGNORECASE,
    ...
    re.compile(
        r"(?:\b(?:weißt|weisst)\s+du(?:\s+(?:genau|eigentlich|denn))*[,]?\s+"
...
CITY_TRAILING_STOP_RE = re.compile(
    ...
    r"zuhause|zu\s+hause|daheim|wohnhaft|ansässig|ansaessig|gemeldet|registriert|geworden|"
...
def extract_residence_city(text: str) -> str:
    ...
    direct_residence_before_current_forward_label = _direct_residence_before_current_forward_label(source)
    if direct_residence_before_current_forward_label:
        return direct_residence_before_current_forward_label
...
def _has_explicit_residence_multiplicity(source: str) -> bool:
    ...
    if direct_status_pair:
        ...
        if re.match(
            ...
            return False
        if not re.match(
            r"(?i)^(?:jetzt|nun(?:mehr)?|aktuell\w*|derzeit\w*|gegenwärtig|gegenwaertig|"
    ...
    if re.search(
        ...
        return True
    if re.search(
        r"\b(?:wohne|wohnen|lebe|leben)\b\s+(?:in|bei)\s+"
        r"[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?\s*,\s*"
        r"(?![^,.;!?]*\d)[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?\s+(?:und|sowie|oder)\s+"
        r"(?!(?:arbeite|arbeitest|arbeiten|studier|lern|schlaf|pendl|reis|besuch|"
        r"umgebung|region|nähe|naehe|umland|umkreis|"
        r"übernacht|uebernacht|fahr|geh|komm|mach)\w*\b)"
        r"[A-ZÄÖÜ][\wÄÖÜäöüß .'-]{1,80}?\s*(?=\s*[.!?;,]|$)",
        multiplicity_source,
        re.IGNORECASE,
...
def _has_conflicting_residence_address_targets(source: str) -> bool:
    ...
    address_patterns = (
        ...
        re.compile(
            ...
            r"(?:ist|lautet|liegt|befindet\s+sich)\s+"
            rf"(?:(?:heute|jetzt|nun|derzeit|{_RESIDENCE_LABEL_CURRENT_QUALIFIER})\s+)*"
            rf"(?:(?:in|bei)\s+)?{city_capture}",
            re.IGNORECASE,
        ),
        re.compile(
...
def _has_historical_residence_suffix(source: str, city_end: int) -> bool:
    ...
    return bool(
        re.match(
            ...
            r"(?:in\s+(?:der|dem)|an\s+(?:der|dem)|wo)\s+(?:ich|wir)\s+"
            r"(?:wohne|wohnen|lebe|leben)\b\s*,?\s*(?:aber|doch|jedoch)\s+"
            r"(?:nicht\s+(?:mehr|l(?:änger|aenger))(?:\s+(?:dort|hier|da))?|früh\w*|frueh\w*|ehemals|damals|vormalig\w*)\b\s*$",
            sentence,
        )
    )
