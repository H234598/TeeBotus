...
def fail_invalid_due_proactive_outbox_items(account_store: AccountStore, account_id: str, *, now: datetime | None = None) -> tuple[str, ...]:
    return _fail_invalid_proactive_outbox_datetime_items(account_store, account_id, field_name="due_at", now=now)


def fail_invalid_retry_at_proactive_outbox_items(account_store: AccountStore, account_id: str, *, now: datetime | None = None) -> tuple[str, ...]:
    ...
    *,
    ...
    with _account_proactive_outbox_lock(account_store, account_id):
        ...
        for item in rows:
            ...
            if str(item.get("status") or "queued").strip().casefold() != "queued":
                continue
            timestamp_value = str(item.get(field_name) or "").strip()
            if not timestamp_value or _parse_proactive_datetime(timestamp_value) is not None:
                continue
            item_id = str(item.get("id") or "").strip()
            if not item_id:
                continue
            ...
            if not isinstance(history, list):
                history = []
                item["status_history"] = history
            history.append({"at": timestamp, "status": "failed", "reason": f"invalid_{field_name}"})
            failed_ids.append(item_id)
        if failed_ids:
            account_store.write_proactive_outbox(account_id, rows)
        return tuple(failed_ids)
    ...
    *,
    ...
    for item in due_proactive_outbox_items(account_store, account_id, now=resolved_now):
        ...
        if isinstance(decision.route, Mapping) and decision.route != route:
            ...
            sender = _sender_for_channel(senders, channel)
            if sender is _SENDER_NOT_FOUND:
                result_reason = _persist_claimed_item_failure(
                    account_store,
                    account_id,
                    item_id,
                    reason=f"missing_sender:{channel}",
                    result_reason="missing_sender",
                    now=resolved_now,
                )
