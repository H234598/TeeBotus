...
def main(argv: Sequence[str] | None = None, *, opener: Any | None = None) -> int:
    ...
    parser.add_argument("--state-db", default="", help="SQLite state DB for live cooldown/usage data. Defaults to XDG state dir when --live is used.")
    args = parser.parse_args(list(argv) if argv is not None else None)

    kwargs = {"pool_name": args.pool, "live": args.live}
    if args.config:
