...
def _health_summary(*, command_ok: bool, parsed_runtime: dict[str, Any], qdrant: dict[str, Any], qdrant_unit: dict[str, Any]) -> dict[str, Any]:
    ...
    effective_actionable_status_counts = {
        status: max(
            ...
            _safe_int(declared_actionable_status_counts.get(status, 0)),
        )
        for status in PROBLEM_STATUSES
    }
    if has_health_classification:
        for status in PROBLEM_STATUSES:
            ...
            if source_count > informational_count:
                effective_actionable_status_counts[status] = max(
                    ...
                    source_count - informational_count,
                )
    parsed_actionable_problem_count = sum(effective_actionable_status_counts.values())
    actionable_problem_count = max(
        0,
        declared_actionable_problem_count,
        parsed_actionable_problem_count,
        sum(declared_actionable_status_counts.values()),
    )
    ...
    severe_count = sum(
        _safe_int(effective_actionable_status_counts.get(status, 0))
...
def _repo_status(repo_root: Path) -> dict[str, Any]:
    ...
    porcelain_header = next((line for line in porcelain_lines if line.startswith("## ")), "")
    status["dirty"] = (
        not _status_query_ok(branch)
        or not _status_query_ok(commit)
        or not _status_query_ok(porcelain)
        or not status["commit"]
        or not porcelain_header
        or "<truncated>" in porcelain_lines
        or any(line and not line.startswith("## ") for line in porcelain_lines)
    )
    status["ahead_behind"] = porcelain_header[3:].strip()
    return status
