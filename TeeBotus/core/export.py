...
SEMANTIC_INDEX_EXPORT_NOTE = {
    "semantic_memory_index": {
        ...
        "qdrant_payload": "Qdrant stores memory IDs, vectors, schema, schema_version, instance scope, derived account_scope, and embedding metadata only; no raw account_id, plaintext, clinical categories, timestamps, or content hashes.",
...
def export_account_data_from_store(account_store: Any, account_id: str, fmt: str) -> ExportResult:
    ...
    payload["files"] = {
        ...
        "Proactive_Dispatch_Results.jsonl": _redact_data(account_store.read_proactive_dispatch_results(account_id)),
    }
    return _emit_payload(account_id, payload, fmt)
