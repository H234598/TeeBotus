...
- Der Collector uebergibt bei einem Repo unter `/home/teladi/...` explizit `/home/teladi/.codex/sessions` und `/home/teladi/.codex-agents`, damit `User=root` nicht versehentlich nur `/root/.codex/sessions` beobachtet. Der Agenten-Root bleibt absichtlich dynamisch; der Importfilter nimmt nur JSONL-Dateien mit `sessions` im Pfad, neue Agentenverzeichnisse werden aber ohne Unit-Neurendering erfasst.
...
- `teebotus-codex-history-collector --collector-timer` rendert einen taeglichen Oneshot-Fallback mit `--limit 1000`, `--post-index`, `--dispatch` und `--dispatch-limit 0` (`0` = alle wartenden Outbox-Eintraege); kurze Testintervalle und kleinere Limits muessen explizit gesetzt werden, normaler Live-Betrieb laeuft ueber den persistenten Follow-Collector.
- Der Fallback-Timer nutzt bewusst eine eigene Service-Datei `teebotus-codex-history-collector-scan.service`, damit er den persistenten Follow-Service `teebotus-codex-history-collector.service` nicht ueberschreibt.
