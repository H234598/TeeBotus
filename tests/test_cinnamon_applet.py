...
def test_cinnamon_applet_runtime_parser_treats_route_error_as_error() -> None:
    parsed = parse_runtime_status(
        """
[LLM-Routen und Backends]
structured_decision=Demo status=enabled route_status=unavailable fallback=local route_error=provider_unavailable
"""
    )
...
def test_cinnamon_applet_runtime_parser_suppresses_route_error_with_verified_fallback() -> None:
    parsed = parse_runtime_status(
        """
[LLM-Routen und Backends]
structured_decision=Demo status=enabled route_status=unavailable fallback=local effective_status=configured route_error=provider_unavailable
"""
    )
...
@pytest.mark.parametrize(
    "response_url",
    [
        "http://localhost:6333/collections/demo/points/count",
        "http://[::1]:6333/collections/demo/points/count",
    ],
)
def test_cinnamon_applet_qdrant_point_count_accepts_local_origin_aliases(monkeypatch, response_url) -> None:
    class FakeResponse:
        status = 200
