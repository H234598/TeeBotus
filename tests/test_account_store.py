...
def test_account_store_refuses_manifest_for_orphaned_sqlite_sidecar(tmp_path, monkeypatch) -> None:
    sqlite_path = tmp_path / "orphaned-memory.sqlite3"
    sqlite_path.with_name(f"{sqlite_path.name}-wal").write_bytes(b"uncheckpointed payload")
    monkeypatch.setenv("TEEBOTUS_ACCOUNT_MEMORY_BACKEND", "sqlite")
    monkeypatch.setenv("TEEBOTUS_ACCOUNT_MEMORY_SQLITE_PATH", str(sqlite_path))
    secret_provider = SecretToolInstanceSecretProvider(create_if_missing=True)
    ...
    monkeypatch.setattr(
        secret_provider,
        "_store",
        lambda _instance, _purpose, _secret: pytest.fail("orphaned SQLite sidecar must not record a key"),
    )
...
def test_sql_account_collection_merge_refuses_partial_rows(tmp_path):
    class PartialReadCollectionBackend:
        def __init__(self, partial_account: str) -> None:
            self.partial_account = partial_account
            self.last_collection_read_error = ""
            self.last_collection_skipped = 0
            self.write_calls = 0

        def read_collection(self, account_id: str, _collection: str) -> list[dict[str, object]]:
            self.last_collection_read_error = "one payload could not be decrypted" if account_id == self.partial_account else ""
            self.last_collection_skipped = 1 if account_id == self.partial_account else 0
            return [{"id": f"row-{account_id[0]}", "status": "queued"}]
    ...
    for partial_account in (source_account_id, target_account_id):
        backend = PartialReadCollectionBackend(partial_account)
        store._account_memory_backend = backend
        with pytest.raises(AccountStoreError, match="collection.*unreadable"):
...
def test_sqlite_memory_backend_rejects_symlinked_primary_without_fallback(tmp_path):
    primary_path = tmp_path / "primary.sqlite3"
    outside = tmp_path / "outside.sqlite3"
    outside.write_bytes(b"not a database")
    primary_path.symlink_to(outside)
...
def test_account_memory_fallback_marks_nonempty_secondary_names_for_repair() -> None:
    class Backend:
        def __init__(self, *, fail_names: bool = False) -> None:
            self.fail_names = fail_names
            self.collections: dict[tuple[str, str], list[dict[str, str]]] = {}
    ...
    assert backend.read_collection_names(account_id) == (collection,)
    assert account_id in backend._failed_collection_name_reads
    assert "requires repair" in backend.fallback_sync_error_for_account(account_id)
...
def test_account_memory_fallback_marks_missing_mirror_replace_stale() -> None:
    class Backend:
    ...
    account_id = "a" * 128
    collection = "codex_history_dispatch_results"
    primary = Backend([{"id": "row_1", "status": "pending"}])
    fallback = Backend([])
    backend = WarningFallbackAccountMemoryBackend(primary, fallback, label="Demo:sqlite")

    assert backend.replace_collection_item(account_id, collection, "row_1", {"id": "row_1", "status": "sent"}) is True
    assert primary.rows == [{"id": "row_1", "status": "sent"}]
    assert fallback.rows == []
    assert (account_id, collection) in backend._stale_fallback_collections
    assert (account_id, collection) in backend._fallback_sync_failed_collections


def test_account_memory_fallback_does_not_mirror_partial_primary_replace() -> None:
    class Backend:
    ...
    account_id = "a" * 128
    collection = "codex_history_dispatch_results"
    primary = Backend([{"id": "row_1", "status": "pending"}], read_error="row_2 could not be decrypted")
