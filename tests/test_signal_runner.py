...
def test_signal_cli_api_account_preflight_groups_localhost_and_loopback(monkeypatch, tmp_path) -> None:
    accounts = (
        AccountRunConfig(
            instance_name="Demo",
            channel="signal",
...
def test_signal_account_health_groups_localhost_and_loopback(monkeypatch, tmp_path) -> None:
    accounts = (
        AccountRunConfig(
            instance_name="Demo",
            channel="signal",
    ...
    assert [item.warning for item in health] == ["", ""]
    assert health[1].router == "shared_with:Demo/signal:1"
    assert [item.registered for item in health] == [True, True]
    assert calls == ["http://localhost:8080"]
