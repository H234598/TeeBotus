...
def test_gemini_interactions_client_extracts_content_delta_wrappers(monkeypatch: pytest.MonkeyPatch) -> None:
    class Interaction:
        id = "interaction-content-delta"
        output_text = ""
        outputs = [
            {
                "type": "content.delta",
                "delta": {"type": "text", "text": "Aus Delta"},
            }
        ]
        usage = {"input_tokens": 5, "output_tokens": 4}
    ...
    assert response.text == "Aus Delta"
    assert response.response_id == "interaction-content-delta"
    assert response.usage == {"input_tokens": 5, "output_tokens": 4}
...
def test_gemini_interactions_client_extracts_step_start_payload(monkeypatch: pytest.MonkeyPatch) -> None:
    class Interaction:
        id = "interaction-step-start"
        steps = [
            {
                "event_type": "step.start",
                "step": {"type": "text", "text": "Step Start"},
            }
        ]
        usage = {"input_tokens": 5, "output_tokens": 4}
    ...
    assert response.text == "Step Start"
    assert response.response_id == "interaction-step-start"
    assert response.usage == {"input_tokens": 5, "output_tokens": 4}
...
def test_gemini_interactions_client_sums_nested_modality_mapping_usage_for_budget(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    reset_gemini_free_tier_budget_state()
    calls: list[str] = []
