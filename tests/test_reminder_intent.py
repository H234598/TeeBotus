...
def test_parse_reminder_accepts_delayed_bescheid_wording() -> None:
    ...
    assert intent.due_at == "2026-06-16T09:00:00+00:00"
    assert intent.subject == "wegen der Tabletten"


def test_parse_reminder_removes_early_daypart_from_subject() -> None:
    intent = parse_reminder_intent(
        "Erinnere mich morgen früh an Sport",
        now=fixed_now(),
    )
...
def test_parse_reminder_removes_time_preposition_from_subject() -> None:
    intent = parse_reminder_intent(
        "Erinnere mich für morgen an den Termin",
        now=fixed_now(),
    )
...
def test_parse_reminder_accepts_do_not_forget_wording() -> None:
    intent = parse_reminder_intent(
        "Bitte nicht vergessen, mich morgen an den Termin zu erinnern",
        now=fixed_now(),
    )
...
def test_parse_reminder_removes_infinitive_tail_from_subject() -> None:
    intent = parse_reminder_intent(
        "Kannst du mich morgen an den Termin zu erinnern",
        now=fixed_now(),
    )
...
def test_parse_reminder_uses_default_subject_for_bare_named_date() -> None:
    intent = parse_reminder_intent(
        "Erinnere mich an den 20. Juni um 9.",
        now=fixed_now(),
    )
...
def test_parse_reminder_uses_default_subject_for_bare_named_date_without_time() -> None:
    intent = parse_reminder_intent(
        "Erinnere mich an den 20. Juni.",
        now=fixed_now(),
    )
...
def test_structured_interval_recurrence_can_start_without_absolute_time(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("TEEBOTUS_PROACTIVE_AGENT_INSTANCES", "Depressionsbot")
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="structured-reminder-interval")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
