...
def test_proactive_cycle_isolates_account_discovery_errors_per_instance(tmp_path) -> None:
    instances_dir = tmp_path / "instances"
    broken_dir = instances_dir / "BrokenInstance" / "data" / "accounts"
    healthy_dir = instances_dir / "HealthyInstance" / "data" / "accounts"
    broken_dir.mkdir(parents=True)
    ...
    class BrokenDiscoveryStore:
        @property
        def accounts_dir(self):
            raise AccountStoreError("account directory unavailable")

    class EmptyStore:
        accounts_dir = healthy_dir / "accounts"

    def factory(_root, instance_name):
        return BrokenDiscoveryStore() if instance_name == "BrokenInstance" else EmptyStore()
    ...
    assert report["ok"] is False
    assert report["instances"][0]["error"] == "AccountStoreError: account directory unavailable"
    assert report["instances"][1]["accounts"] == []
