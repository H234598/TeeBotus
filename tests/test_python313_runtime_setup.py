...
def test_python313_runtime_setup_runs_expected_commands(monkeypatch, tmp_path: Path) -> None:
    ...
    assert calls[3][:5] == [str(tmp_path / "venv" / "bin" / "python"), "-m", "pip", "install", "--no-deps"]
    assert calls[4][:2] == [str(tmp_path / "venv" / "bin" / "python"), str(setup_python313.REPO_ROOT / "scripts" / "install_adapter_deps.py")]
