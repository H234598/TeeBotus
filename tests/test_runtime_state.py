...
def test_runtime_state_store_rejects_symlinked_account_state_root(tmp_path):
    ...
    assert "unsafe account state root" in state.llm_state_persistence_error
...
def test_runtime_state_store_rejects_symlinked_account_state_directory(tmp_path):
    data_dir = tmp_path / "Bot" / "data"
    accounts_root = data_dir / "accounts" / "accounts"
    accounts_root.mkdir(parents=True)
    outside = tmp_path / "outside-account-state-directory"
    outside.mkdir()
    (accounts_root / ACCOUNT_ID).symlink_to(outside, target_is_directory=True)
    state = RuntimeStateStore(data_dir, instance_name="Bot", secret_provider=StaticSecretProvider(b"s" * 32))

    with pytest.raises(AccountStoreError, match="unsafe account state directory"):
        state.set_previous_response_id("Bot", ACCOUNT_ID, "response-id")
...
def test_runtime_state_store_rejects_cross_account_llm_state_hardlink(tmp_path):
    data_dir = tmp_path / "Bot" / "data"
    provider = StaticSecretProvider(b"s" * 32)
    other_account_id = "b" * 128
    account_store = AccountStore(data_dir / "accounts", "Bot", secret_provider=provider)
    ...
    target_dir = account_store.account_dir(ACCOUNT_ID)
    target_dir.mkdir(parents=True, exist_ok=True)
    os.link(account_store.account_dir(other_account_id) / LLM_STATE_FILENAME, target_dir / LLM_STATE_FILENAME)
    state = RuntimeStateStore(data_dir, instance_name="Bot", secret_provider=provider)
...
def test_runtime_state_store_keeps_unchanged_response_after_failed_rewrite(tmp_path):
    secret = b"s" * 32
    data_dir = tmp_path / "Bot" / "data"
    account_store = AccountStore(data_dir / "accounts", "Bot", secret_provider=StaticSecretProvider(secret))
    account_store.write_llm_state(
    ...
    state.set_previous_response_id("Bot", ACCOUNT_ID, "resp-old", provider="openai", model="gpt-5.5")
    assert ("Bot", ACCOUNT_ID) in state.pending_previous_response_resets

    recovering_provider.available = True

    assert state.get_previous_response_id("Bot", ACCOUNT_ID, provider="openai", model="gpt-5.5") == "resp-old"
    assert ("Bot", ACCOUNT_ID) not in state.pending_previous_response_resets
