...
def test_residence_resolution_rejects_deictic_non_city_clause() -> None:
    ...
    assert extract_residence_city("Weißt du, wo ich wohne? Berlin.") == "Berlin"
    assert extract_residence_city("Weisst du eigentlich, wo ich lebe? Potsdam.") == "Potsdam"
    assert extract_residence_city("Weißt du denn, wo ich wohne? Berlin.") == "Berlin"
    assert extract_residence_city("Weißt du eigentlich genau, wo ich wohne? Berlin.") == "Berlin"
    assert extract_residence_city("Weißt du, wo ich wohne? jetzt Berlin.") == "Berlin"
    assert extract_residence_city("Weißt du, wo ich wohne? aktuell in Berlin.") == "Berlin"
    assert extract_residence_city("Weißt du, wo ich wohne? Berlin und Hamburg.") == ""
    assert extract_residence_city("Weißt du, wo ich wohne? morgen Berlin.") == ""
    assert extract_residence_city("Weißt du, wo ich wohne? später Berlin.") == ""
...
def test_extract_residence_city_from_address_conflict_forms() -> None:
    ...
    assert extract_residence_city("Ich wohne in Berlin, meine Wohnadresse ist Potsdam.") == ""
    assert extract_residence_city("Berlin, meine aktuelle Adresse ist Hamburg.") == ""
    assert extract_residence_city("Berlin, meine aktuelle Adresse ist Berlin.") == "Berlin"
    assert extract_residence_city("Berlin, meine aktuelle Meldeadresse ist Berlin.") == "Berlin"
    assert extract_residence_city("Berlin, wo ich lebe, und Hamburg, wo meine Adresse ist.") == ""
    assert extract_residence_city("Berlin, wo ich lebe, und Berlin, wo meine Adresse ist.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. In Hamburg habe ich meine aktuelle Adresse.") == ""
    assert extract_residence_city("Ich wohne in Berlin. In Berlin habe ich meine aktuelle Adresse.") == "Berlin"
...
def test_extract_residence_city_from_current_location_label() -> None:
    ...
    assert extract_residence_city("Ich wohnte in Berlin, heute Hamburg.") == "Hamburg"
    assert extract_residence_city("Wir lebten in Berlin, nun Potsdam.") == "Potsdam"
    assert extract_residence_city("Ich wohnte in Berlin, heute arbeite in Hamburg.") == ""
...
def test_extract_residence_city_rejects_conflicting_private_addresses() -> None:
    ...
    assert extract_residence_city("Ich wohne aktuell in Berlin. Mein Wohnort ist Hamburg.") == ""
    assert extract_residence_city("Ich wohne derzeit in Berlin. Hamburg ist mein Wohnort.") == ""
    assert extract_residence_city("Ich wohne aktuell in Berlin. Jetzt ist mein Wohnort Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein Wohnort ist aktuell Berlin. Meine Adresse ist Hamburg.") == ""
    ...
    assert extract_residence_city("Ich wohne in Berlin und meine Adresse ist ebenfalls Berlin.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und meine Wohnadresse ist auch Berlin.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und meine Arbeitsadresse ist Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und meine Geschäftsadresse ist Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und meine Postadresse ist Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und meine Meldeadresse ist Berlin.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber meine Adresse Hamburg.") == ""
    assert extract_residence_city(
        "Hamburg war früher mein Wohnort, jetzt ist es Berlin, aber meine Adresse Potsdam."
    ) == ""
    assert extract_residence_city(
        "Hamburg war früher mein Wohnort, jetzt ist es Berlin, doch meine Wohnadresse Potsdam."
    ) == ""
...
def test_extract_residence_city_handles_current_city_after_residence_change() -> None:
    ...
    assert extract_residence_city("Vorher Berlin, jetzt arbeite in Hamburg.") == ""
    assert extract_residence_city("Ich war früher in Berlin, heute Hamburg.") == "Hamburg"
    assert extract_residence_city("Ich war früher in Berlin, heute in Hamburg.") == "Hamburg"
    assert extract_residence_city("Ich war früher in Berlin, heute arbeite in Hamburg.") == ""
    assert extract_residence_city("In Berlin wohnhaft, heute Hamburg.") == "Hamburg"
    assert extract_residence_city("In Berlin wohnhaft, heute in Hamburg.") == "Hamburg"
    assert extract_residence_city("In Berlin wohnhaft, heute arbeite in Hamburg.") == "Berlin"
...
def test_extract_residence_city_removes_daypart_context() -> None:
    ...
    assert extract_residence_city("Ich wohne in Frankfurt a. M.") == "Frankfurt am Main"
    assert extract_residence_city("Ich wohne in Mülheim a. d. Ruhr.") == "Mülheim an der Ruhr"
    ...
    assert extract_residence_city("Berlin war mein alter Wohnort, jetzt Hamburg.") == "Hamburg"
    ...
    assert extract_residence_city("Berlin ist mein Wohnort. Jetzt Hamburg, aber arbeite dort.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Jetzt Hamburg, dort studiere ich.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Jetzt Hamburg, wo ich arbeite.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Jetzt Hamburg, um dort zu arbeiten.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Heute Hamburg, dort bin ich beruflich.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Heute Hamburg, dort übernachte ich.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Heute Hamburg, dort bin ich wohnhaft.") == "Hamburg"
    ...
    assert extract_residence_city("Berlin ist mein Wohnort. Ab sofort bin ich in Hamburg gemeldet.") == "Hamburg"
    assert extract_residence_city("Berlin ist mein Wohnort. Ab dem 1. Juli bin ich in Hamburg gemeldet.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort, jetzt arbeite ich in Hamburg.") == "Berlin"
    assert extract_residence_city("Berlin war mein alter Wohnsitz, jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Berlin war meine alte Wohnadresse, jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein altes Zuhause war Berlin, mein neues ist Hamburg.") == "Hamburg"
    assert extract_residence_city("Berlin war meine alte Wohnadresse, Hamburg ist meine neue.") == "Hamburg"
    assert extract_residence_city("Mein früherer Wohnort Berlin, mein jetziger Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein alter Wohnsitz Berlin, mein neuer Hamburg.") == "Hamburg"
    assert extract_residence_city("Berlin ist meine alte Wohnadresse, Hamburg meine neue Wohnadresse.") == "Hamburg"
    assert extract_residence_city(
        "Berlin ist meine alte Wohnadresse, Hamburg meine neue Wohnadresse; Meldeadresse Hamburg."
    ) == "Hamburg"
    assert extract_residence_city(
        "Berlin ist meine alte Wohnadresse, Hamburg meine neue Wohnadresse; Meldeadresse München."
    ) == ""
...
def test_extract_residence_city_rejects_secondary_residence_without_primary() -> None:
    ...
    assert extract_residence_city("Ich wohne in Berlin. Meine neue Adresse ist Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Meine aktuelle Adresse ist Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Meine neue Meldeadresse ist Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin, aber Hamburg ist meine aktuelle Adresse.") == ""
    assert extract_residence_city("Ich wohne in Berlin, aber Berlin ist meine aktuelle Adresse.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber Hamburg meine aktuelle Adresse.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Hamburg, wo meine aktuelle Adresse ist.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Hamburg, wo meine aktuelle Meldeadresse ist.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Berlin, wo meine aktuelle Adresse ist.") == "Berlin"
    ...
    assert extract_residence_city("Ich wohne in Berlin und Berlin meine aktuelle Adresse.") == "Berlin"
    ...
    assert extract_residence_city("Ich wohne in Berlin, aber meine Adresse ist aktuell Hamburg.") == ""
    assert extract_residence_city("Ich wohne aktuell in Berlin, meine Adresse ist derzeit Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin, aber meine Adresse ist jetzt Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin, aber meine Adresse ist heute Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin, aber meine Adresse ist inzwischen Hamburg.") == ""
    ...
    assert extract_residence_city("Berlin ist mein Wohnort, Hamburg meine private Adresse.") == ""
    assert extract_residence_city("Berlin ist mein Wohnort, Berlin meine private Adresse.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, Hamburg meine aktuelle private Adresse.") == ""
    assert extract_residence_city("Ich wohne in Berlin, Berlin meine aktuelle private Adresse.") == "Berlin"
...
def test_extract_residence_city_handles_inverted_relative_residence_references() -> None:
    ...
    assert extract_residence_city("Ich wohne nicht mehr in Berlin, inzwischen wohnhaft in Hamburg.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin, jetzt arbeite ich in Hamburg.") == "Berlin"
...
def test_extract_residence_city_handles_direct_home_relationships() -> None:
    ...
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Wohnort.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Hauptwohnsitz.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist jetzt mein Wohnort.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist künftig mein Wohnort.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein ehemaliger Wohnort.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist vorübergehend mein Wohnort.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein vorübergehender Wohnort.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Wohnort, aber nur vorübergehend.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Zuhause, aber nur am Wochenende.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Wohnort seit gestern.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Wohnort, vorher war es Berlin.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Wohnort, früher war es Berlin.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Hamburg ist mein Wohnort, war früher Berlin.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Mein aktueller Wohnort ist Hamburg.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Mein Wohnort ist jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin. Mein Wohnort ist Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin. Mein vorübergehender Wohnort ist Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Mein Wohnort ist vorübergehend Hamburg.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Wohnort. Hamburg ist mein Wohnort.") == ""
    assert extract_residence_city("Hamburg ist mein vorübergehender Wohnort.") == "Hamburg"
...
def test_extract_residence_city_handles_temporal_label_forms() -> None:
    ...
    assert extract_residence_city("Hamburg war früher mein Wohnort, jetzt ist es Berlin.") == "Berlin"
    assert extract_residence_city("Mein früherer Wohnort war Hamburg, jetzt ist es Berlin.") == "Berlin"
    assert extract_residence_city("Hamburg war früher mein Wohnort, jetzt befindet es sich in Berlin.") == "Berlin"
    assert extract_residence_city("Mein früherer Wohnsitz Hamburg, jetzt ist es Berlin.") == "Berlin"
    assert extract_residence_city("Hamburg ist mein früherer Wohnort, nun ist es Berlin.") == "Berlin"
    ...
    assert extract_residence_city("Mein neues Zuhause Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein jetziges Zuhause Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein künftiges Zuhause Hamburg.") == ""
    assert extract_residence_city("Mein altes Zuhause war Berlin, jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein alter Wohnort war Berlin, inzwischen Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein früherer Wohnort Berlin, jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein altes Zuhause Berlin, heute Hamburg.") == "Hamburg"
    assert extract_residence_city("Meine alte Wohnadresse Berlin, jetzt arbeite in Hamburg.") == ""
    assert extract_residence_city("Meine alte Wohnadresse war Berlin, jetzt arbeite in Hamburg.") == ""
    assert extract_residence_city("Wohnadresse alt Berlin, jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Adresse früher Berlin, heute Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnadresse Berlin, aktuell Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort Berlin, neu Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort alt: Berlin, neu: Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort alt: Berlin, neu: arbeite in Hamburg.") == ""
    assert extract_residence_city("Hamburg, wo ich jetzt wohne; vorher Berlin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich derzeit lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, in der ich jetzt wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, in dem ich derzeit lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich momentan wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, in dem ich zurzeit lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, in der ich weiterhin wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, in welcher ich jetzt wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, worin ich derzeit lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, in dem ich wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, in der ich wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, in welcher ich wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, worin ich wohne.") == "Hamburg"
    assert extract_residence_city("In Hamburg, in dem ich wohne.") == "Hamburg"
    ...
    assert extract_residence_city(
        "Ich wohne in Berlin, aber in Hamburg bin ich am Wochenende."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin, in Hamburg bin ich unter der Woche."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin, aber in Hamburg bin ich nur tagsüber."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin, aber in Hamburg schlafe ich manchmal."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin, in Hamburg übernachte ich manchmal."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin, in Hamburg lebe ich nur bei Besuch."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin; in Hamburg bin ich zuhause."
    ) == ""
    assert extract_residence_city(
        "In Berlin wohne ich, aber in Hamburg übernachte ich manchmal."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin und halte mich beruflich in Hamburg auf."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin und halte mich in Hamburg auf."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin und halte mich tagsüber in Hamburg auf."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin und schlafe manchmal in Hamburg."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin, aber schlafe manchmal in Hamburg."
    ) == "Berlin"
    assert extract_residence_city(
        "Ich wohne in Berlin und verbringe manchmal Nächte in Hamburg."
    ) == "Berlin"
    assert extract_residence_city(
        "In Berlin wohne ich und in Hamburg bin ich zuhause."
    ) == ""
    assert extract_residence_city(
        "In Berlin wohne ich und in Hamburg wohne ich."
    ) == ""
    assert extract_residence_city(
        "In Berlin wohne ich. In Hamburg wohne ich."
    ) == ""
    assert extract_residence_city(
        "In Berlin wohne ich. Jetzt in Hamburg wohne ich."
    ) == "Hamburg"
    assert extract_residence_city(
        "In Berlin wohne ich. Seit 2024 in Hamburg wohne ich."
    ) == "Hamburg"
    assert extract_residence_city(
        "In Berlin wohne ich. Vorher in Hamburg wohne ich."
    ) == "Berlin"
    assert extract_residence_city(
        "In Berlin wohne ich. Später in Hamburg wohne ich."
    ) == "Berlin"
    assert extract_residence_city(
        "In Berlin wohne ich. Vielleicht in Hamburg wohne ich."
    ) == "Berlin"
    assert extract_residence_city(
        "In Berlin wohne ich. In Hamburg wohne ich vielleicht."
    ) == "Berlin"
    assert extract_residence_city("In Hamburg wohne ich früher.") == ""
    assert extract_residence_city("In Hamburg wohne ich später.") == ""
    assert extract_residence_city("In Hamburg wohne ich ab morgen.") == ""
    assert extract_residence_city("In Hamburg wohne ich künftig.") == ""
    assert extract_residence_city("In Hamburg wohne ich seit 2024.") == "Hamburg"
    assert extract_residence_city("Hamburg; wo ich jetzt wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg: wo ich jetzt wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg – wo ich jetzt wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich seit 2020 lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich wohnhaft bin.") == "Hamburg"
    assert extract_residence_city("Hamburg, in der ich aktuell gemeldet bin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich zu Hause bin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich offiziell gemeldet bin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich amtlich registriert bin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich polizeilich gemeldet bin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich tatsächlich wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich meinen Wohnsitz habe.") == "Hamburg"
    assert extract_residence_city("Hamburg, in dem mein Wohnsitz liegt.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo sich mein Wohnsitz befindet.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich meine Meldeadresse habe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo mein aktueller Wohnsitz ist.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich offiziell meinen Wohnsitz habe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo sich mein aktueller Wohnsitz befindet.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich meinen Arbeitsort habe.") == ""
    assert extract_residence_city("Hamburg, wo ich meine Arbeitsadresse habe.") == ""
    assert extract_residence_city("Hamburg, wo ich überwiegend lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich meistens wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich beheimatet bin.") == "Hamburg"
    assert extract_residence_city("Wohnort: Hamburg; Adresse: Musterstraße 1, Berlin.") == ""
    assert extract_residence_city("Wohnort: Berlin; Adresse: Berlin.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin; Wohnadresse: Berlin.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin; Arbeitsadresse: Hamburg.") == "Berlin"
    assert extract_residence_city("Mein Wohnort ist Berlin und meine Adresse ist Berlin.") == "Berlin"
    assert extract_residence_city("Mein Wohnort ist Berlin und meine Anschrift ist Berlin.") == "Berlin"
    assert extract_residence_city("Mein Wohnort ist Berlin und meine Postadresse ist Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Hamburg; Adresse: Musterstraße 1, Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Hamburg, Adresse: Musterstraße 1, Berlin.") == ""
    assert extract_residence_city("Wohnort: Hamburg, Adresse: Musterstraße 1, Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Hamburg, Adresse: Musterstraße 1, 10115 Berlin.") == ""
    assert extract_residence_city("Wohnort: Hamburg, Adresse: Musterstraße 1, Berlin (Deutschland).") == ""
    assert extract_residence_city("Berlin, wo ich wohne, und Hamburg, wo ich arbeite.") == "Berlin"
    assert extract_residence_city("Berlin, wo ich arbeite, und Hamburg, wo ich wohne.") == "Hamburg"
    assert extract_residence_city("Berlin, wo ich wohne, und Hamburg, wo ich wohne.") == ""
    assert extract_residence_city(
        "Ich wohne in Berlin, wo ich arbeite, und in Hamburg, wo ich wohne."
    ) == ""
    assert extract_residence_city("Hamburg, wo ich ansässig lebe.") == ""
    assert extract_residence_city("Hamburg, wo ich mit meiner Familie lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich bei meinen Eltern wohne.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo wir zusammen mit unseren Kindern leben.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich nur vorübergehend lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich nur vorübergehend mit meiner Familie lebe.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich mit meinem Studium lebe.") == ""
    assert extract_residence_city("In Hamburg habe ich meine Meldeadresse.") == "Hamburg"
    assert extract_residence_city("Bei Hamburg habe ich meinen Meldesitz.") == "Hamburg"
    assert extract_residence_city("In Hamburg habe ich meine aktuelle Meldeadresse.") == "Hamburg"
    assert extract_residence_city("In Hamburg habe ich meine offizielle Meldeanschrift.") == "Hamburg"
    assert extract_residence_city("In Hamburg habe ich meine Wohnadresse.") == "Hamburg"
    assert extract_residence_city("In Hamburg habe ich meine private Adresse.") == "Hamburg"
    assert extract_residence_city("In Hamburg habe ich meine Arbeitsadresse.") == ""
    assert extract_residence_city("In Hamburg habe ich meine Geschäftsadresse.") == ""
    assert extract_residence_city("In Hamburg habe ich meine ehemalige Meldeadresse.") == ""
    assert extract_residence_city("Ich wohne in Hamburg (Berlin).") == ""
    assert extract_residence_city("Wohnort Hamburg (Arbeitsort).") == ""
    assert extract_residence_city("Hamburg (Wohnort).") == "Hamburg"
    assert extract_residence_city("Hamburg (Wohnsitz).") == "Hamburg"
    assert extract_residence_city("Hamburg (Meldeadresse).") == "Hamburg"
    assert extract_residence_city("Mein Wohnort ist München (Bayern).") == "München"
    assert extract_residence_city("Ich wohne in Hamburg (Norddeutschland).") == "Hamburg"
    assert extract_residence_city("Meine Adresse ist Köln (NRW).") == "Köln"
    assert extract_residence_city("Mein Wohnort ist München (Schwabing).") == "München"
    assert extract_residence_city("München (Bayern), wo ich jetzt wohne.") == "München"
    assert extract_residence_city("Hamburg, wo ich jetzt wohne, vorher Berlin.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich jetzt wohne, aber in Berlin arbeite.") == "Hamburg"
    assert extract_residence_city("Hamburg, wo ich jetzt arbeite; vorher Berlin.") == ""
    assert extract_residence_city("Hamburg, wo ich früher wohnte.") == ""
    assert extract_residence_city("Hamburg, wo ich früher wohnhaft bin.") == ""
    assert extract_residence_city("Hamburg, wo ich künftig wohnen werde.") == ""
    assert extract_residence_city("Wohnort Berlin, neuer Wohnort Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, neuer Wohnort: Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin. Jetzt: Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein Wohnsitz: Berlin. Aktuell: Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort = Berlin, aktuell = Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, aktuell gemeldet in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seit heute Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seit heute in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seit 2024 Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seit 2024 in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seit zwei Jahren Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seit zwei Jahren in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, ab jetzt Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, ab jetzt in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, nunmehr Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, momentan in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, weiterhin Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, nach wie vor in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, noch immer Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, immer noch in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, zurzeit Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, seitdem Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, dauerhaft Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, permanent in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, langfristig Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, endgültig in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, unbefristet Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, stabil in Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, zeitweise Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, befristet wohnhaft in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin. Vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, vorläufig in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, vorlaeufig Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, bis auf Weiteres in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, bis übermorgen Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, für zwei Wochen in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, für den Urlaub in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nur am Wochenende in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nur tagsüber in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nur bei Besuch in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, gelegentlich in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, manchmal in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nur werktags in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nur nachts in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nur montags in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, regelmäßig in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, selten in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, zu Weihnachten in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, an Ostern in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, über die Feiertage in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, geschäftlich in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, projektbedingt in Hamburg.") == "Berlin"
    assert extract_residence_city("Zuhause: Berlin, nur werktags in Hamburg.") == "Berlin"
    assert extract_residence_city("Daheim: Berlin, zu Weihnachten in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg werde ich wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg möchte ich wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg soll ich wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg würde ich wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg werde ich bald wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg plane ich zu wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg habe ich vor zu wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg könnte ich wohnen.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg ziehe ich ein.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber in Hamburg ziehe ich um.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber befristet in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber für zwei Wochen in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber bis Jahresende in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber bis zum Ende des Monats in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, aber bis Ende der Woche in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne aktuell in Berlin, aber vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne hier in Berlin, aber vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne nur noch in Berlin, aber vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne ausschließlich in Berlin, aber vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne rein in Berlin.") == "Berlin"
    assert extract_residence_city("Ich wohne bloß in Berlin.") == "Berlin"
    assert extract_residence_city("Ich wohne bloss in Berlin.") == "Berlin"
    assert extract_residence_city("Mein Wohnort ist aktuell Berlin, aber vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Mein Hauptwohnsitz liegt in Berlin, aber befristet in Hamburg.") == "Berlin"
    assert extract_residence_city("Berlin ist mein Zuhause, aber für zwei Wochen in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin - vorübergehend in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort Berlin, aktuell wohnhaft Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein Wohnort ist Berlin, jetzt wohnhaft Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort Berlin, aktuell gemeldet Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort Berlin, aktuell ansässig Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort: Berlin, wohnhaft Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin, gemeldet Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin, ansässig Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin, dauerhaft wohnhaft Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin; wohnhaft Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin. Wohnhaft Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin, nicht mehr wohnhaft in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich bin nicht mehr wohnhaft in Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin, nicht länger wohnhaft in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich bin nicht länger wohnhaft in Hamburg.") == ""
    assert extract_residence_city("Wohnort: Berlin, nie wieder wohnhaft in Hamburg.") == "Berlin"
    assert extract_residence_city("Wohnort: Berlin, nicht wohnhaft in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin, bin in Hamburg wohnhaft.") == ""
    assert extract_residence_city("Ich wohne in Berlin, in Hamburg ansässig.") == ""
    assert extract_residence_city("Ich wohne in Berlin, wohnhaft Hamburg.") == ""
    assert extract_residence_city("Ich wohne in Berlin, jetzt in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin, ab sofort in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin und bin ab sofort in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin, gegenwärtig in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin, gegenwaertig in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin, nunmehr in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Wir wohnen in Berlin und sind gegenwärtig in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin, künftig in Hamburg wohnhaft.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin und bin in Hamburg wohnhaft.") == ""
    assert extract_residence_city("Wir wohnen in Berlin und sind aktuell in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Wir wohnen in Berlin und sind derzeit in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Wir wohnen in Berlin und sind nunmehr in Hamburg wohnhaft.") == "Hamburg"
    ...
    assert extract_residence_city("Ich wohne in Berlin, seit langer Zeit in Hamburg wohnhaft.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Berlin und bin seit geraumer Zeit in Hamburg wohnhaft.") == "Hamburg"
    ...
    assert extract_residence_city("Meine alte Wohnadresse: Berlin, neue Wohnadresse: Hamburg.") == "Hamburg"
    assert extract_residence_city("Meine alte Adresse: Berlin, neue Adresse: Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein früherer Wohnort: Berlin, mein aktueller: Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein früherer Wohnsitz: Berlin; mein jetziger: Hamburg.") == "Hamburg"
    assert extract_residence_city("Ich bin in Hamburg wohnhaft, davor Berlin.") == "Hamburg"
    assert extract_residence_city("Wohnort Hamburg, davor Berlin.") == "Hamburg"
    assert extract_residence_city("In Hamburg wohnhaft, zuvor Berlin.") == "Hamburg"
    assert extract_residence_city("Hamburg wohnhaft, früher Berlin.") == "Hamburg"
    assert extract_residence_city("Aktuell Hamburg, vorher Berlin.") == "Hamburg"
    assert extract_residence_city("Ich wohne in Hamburg (früher Berlin).") == "Hamburg"
    assert extract_residence_city("Wohnort Hamburg (zuvor Berlin).") == "Hamburg"
    assert extract_residence_city("Wohnadresse Berlin, aktuell Hamburg; Meldeadresse Hamburg.") == "Hamburg"
    assert extract_residence_city("Wohnort Berlin, neu Hamburg; Meldeadresse Hamburg.") == "Hamburg"
    ...
    assert extract_residence_city("Wohnort Hamburg, zuvor Berlin.") == "Hamburg"
    assert extract_residence_city("Wohnadresse Hamburg, zuvor Berlin.") == "Hamburg"
    assert extract_residence_city("Mein Wohnort früher Berlin, heute Hamburg.") == "Hamburg"
    assert extract_residence_city("Mein Wohnort früher Berlin, heute arbeite ich in Hamburg.") == ""
    ...
    assert extract_residence_city("Ich wohne in Berlin. Vielleicht wohne ich in Hamburg.") == "Berlin"
    assert extract_residence_city("Ich wohne in Berlin. Wahrscheinlich lebe ich später in Hamburg.") == "Berlin"
...
def test_weather_context_clear_text_skips_legacy_pattern_scan(monkeypatch) -> None:
    def fail_legacy_parser(_text: str) -> str:
        raise AssertionError("clear residence claim entered legacy pattern scan")
    ...
    assert _resolve_residence_city(
        "Ich wohne in Berlin.",
        lambda _prompt, _schema: {"kind": "ambiguous", "city": "", "confidence": 0.0},
    ) == "Berlin"
...
def test_city_comparison_keeps_transliterated_same_city(tmp_path) -> None:
    account_store = store(tmp_path)
    _identity, account_id = prepare_account(account_store)
    calls: list[str] = []
