...
def test_proactive_queue_rejects_missing_required_content_before_write(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    missing_intent = queue_proactive_message(
        account_store,
        account_id,
        category="reminder",
        intent=" ",
    ...
    missing_message = queue_proactive_message(
        account_store,
        account_id,
        category="reminder",
        intent="follow_up",
    ...
    assert missing_intent == ProactiveDecision(False, "missing_intent")
    assert missing_message == ProactiveDecision(False, "missing_message_text")
    assert account_store.read_proactive_outbox(account_id) == []
...
def test_proactive_worker_claim_normalizes_item_id_whitespace(tmp_path) -> None:
    account_store = store(tmp_path)
    account_id = account_store.resolve_or_create_account(telegram_identity_key(1))
    account_store.write_proactive_outbox(
        account_id,
        [{"id": " item_with_padding ", "status": "queued"}],
    )
...
def test_proactive_agent_health_reports_future_dispatching_timestamp(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = telegram_identity_key(1)
    account_id = account_store.resolve_or_create_account(identity)
    account_store.write_proactive_outbox(
        ...
        [
            {
                "id": "pro_future_claim_timestamp",
                "status": "dispatching",
                "dispatching_at": "2026-06-16T12:00:00+00:00",
                "status_history": [{"at": "2026-06-16T12:00:00+00:00", "status": "dispatching", "reason": "worker_claimed"}],
            }
        ],
    )
    ...
    assert health.ok is False
    assert "dispatching outbox item pro_future_claim_timestamp has future claim timestamp" in health.errors
...
def test_outbox_status_update_preserves_invalid_status_history(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    queued = queue_proactive_message(
        ...
        account_id,
        category="reminder",
        intent="corrupt_history",
        message_text="Historie erhalten",
        now=datetime(2026, 6, 15, 12, tzinfo=timezone.utc),
    )
    item_id = queued.reason.removeprefix("queued:")
    item = account_store.read_proactive_outbox(account_id)[0]
    item["status_history"] = {"broken": True}
    account_store.write_proactive_outbox(account_id, [item])

    assert not update_proactive_outbox_item_status(
        account_store,
        account_id,
        item_id,
        status="dispatching",
...
def test_human_review_approval_rejects_invalid_status_history_before_write(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    item_id = decision.reason.removeprefix("review_pending:")
    rows = account_store.read_proactive_outbox(account_id)
    rows[0]["status_history"] = [{"status": "queued", "at": "broken", "reason": "old"}]
    account_store.write_proactive_outbox(account_id, rows)

    approved = approve_proactive_review_item(account_store, account_id, item_id, now=datetime(2026, 6, 15, 10, 30, tzinfo=timezone.utc))

    assert approved == ProactiveDecision(False, "invalid_status_history")
    assert account_store.read_proactive_outbox(account_id)[0]["status"] == "review_pending"
...
def test_reflection_planner_rolls_back_partial_memory_batch(tmp_path, monkeypatch) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    source_id = account_store.append_structured_memory_entry(
        account_id,
        {"id": "mem_goal", "kind": "therapy_goal", "user_text": "Spaziergang planen."},
    )
    ...
    monkeypatch.setattr(account_store, "append_structured_memory_entry", fail_after_two)
    result = run_proactive_reflection_planner(account_store, account_id, now=datetime(2026, 6, 15, 12, tzinfo=timezone.utc))

    assert result.skipped_reason == "memory_persistence_failed"
    assert result.created_memory_ids == ()
    assert result.queued_item_ids == ()
    assert [entry["id"] for entry in account_store.read_memory_entries(account_id)] == [source_id]
    assert account_store.read_proactive_outbox(account_id) == []


def test_reflection_planner_rolls_back_memory_batch_when_queue_fails(tmp_path, monkeypatch) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    monkeypatch.setattr(account_store, "append_proactive_outbox_item", fail_queue)
    result = run_proactive_reflection_planner(account_store, account_id, now=datetime(2026, 6, 15, 12, tzinfo=timezone.utc))

    assert result.skipped_reason == "queue_persistence_failed"
    assert result.created_memory_ids == ()
    assert result.queued_item_ids == ()
    assert [entry["id"] for entry in account_store.read_memory_entries(account_id)] == [source_id]
    assert account_store.read_proactive_outbox(account_id) == []
...
def test_llm_plan_reports_memory_persistence_failure_and_continues(tmp_path, monkeypatch) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    source_id = account_store.append_structured_memory_entry(
        account_id,
        {"kind": "therapy_goal", "user_text": "Spaziergang planen."},
    )

    def fail_memory_write(*_args, **_kwargs):
        raise OSError("memory backend unavailable")

    monkeypatch.setattr(account_store, "append_structured_memory_entry", fail_memory_write)
    result = apply_proactive_llm_plan(
        account_store,
        account_id,
        {
            "schema_version": 1,
            "decisions": [
                {"action": "memory", "kind": "reflection", "text": "Reflexion speichern.", "source_memory_ids": [source_id]},
                {
                    "action": "queue",
                    "category": "reminder",
                    "intent": "follow_up",
                    "message_text": "Morgen kurz nach dem Spaziergang schauen?",
                    "reason_memory_ids": [source_id],
    ...
    assert result.errors == ("decision_0_storage_write_failed",)
    assert len(result.queued_item_ids) == 1


def test_llm_plan_reports_queue_persistence_failure(tmp_path, monkeypatch) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    monkeypatch.setattr(account_store, "append_proactive_outbox_item", fail_queue_write)
    result = apply_proactive_llm_plan(
        account_store,
        account_id,
        {
...
def test_tool_agent_runner_rejects_non_object_noop_arguments(tmp_path) -> None:
    class Client:
        def create_tool_calls(self, _prompt, _instructions, _tools):
            return {
                "tool_calls": [
...
def test_stale_dispatch_claim_with_invalid_attempts_fails_closed(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    queued = queue_proactive_message(
        ...
        account_id,
        category="reminder",
        intent="corrupt_attempts",
        message_text="Nicht still reparieren",
        due_at="2026-06-15T11:00:00+00:00",
        now=datetime(2026, 6, 15, 10, tzinfo=timezone.utc),
    )
    item = account_store.read_proactive_outbox(account_id)[0]
    ...
    assert item["status"] == "failed"
    assert item["dispatch_attempts"] == "kaputt"
    assert item["status_history"][-1]["reason"] == "invalid_dispatch_attempts_after_30_minute_recovery"
...
def test_dispatch_send_error_fails_closed_on_corrupt_snapshot_attempts(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    queued = queue_proactive_message(
        ...
        account_id,
        category="reminder",
        intent="invalid_attempts_snapshot",
        message_text="Nicht erneut senden",
        due_at="2026-06-15T11:00:00+00:00",
        now=now,
    )
    item_id = queued.reason.removeprefix("queued:")
    ...
    assert results[0].item_id == item_id
    assert results[0].status == "failed"
    assert results[0].reason == "invalid_dispatch_attempts"
    assert account_store.read_proactive_outbox(account_id)[0]["status"] == "failed"
...
def test_dispatch_fails_invalid_retry_at_without_sending(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    assert [result.item_id for result in results] == [item_id]
    assert results[0].status == "failed"
    assert results[0].reason == "invalid_retry_at"
    item = account_store.read_proactive_outbox(account_id)[0]
    assert item["status"] == "failed"
    assert item["status_history"][-1]["reason"] == "invalid_retry_at"


def test_dispatch_fails_invalid_recurrence_without_sending(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = signal_identity_key(source_uuid="signal-user")
    account_id = account_store.resolve_or_create_account(identity)
    account_store.update_identity_route(identity, channel="signal", chat_id="+491", chat_type="private", adapter_slot=1)
    ...
    assert [result.item_id for result in results] == [item_id]
    assert results[0].status == "failed"
    assert results[0].reason == "invalid_recurrence"
    item = account_store.read_proactive_outbox(account_id)[0]
    assert item["status"] == "failed"
