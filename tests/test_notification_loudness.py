...
def test_engine_asks_user_to_unmute_bot_messages_once_route_exists(tmp_path) -> None:
    ...
    assert route_state["next_check_at"]
...
def test_scheduler_queues_half_day_notification_loudness_follow_up(tmp_path) -> None:
    account_store = store(tmp_path)
    identity = telegram_identity_key(1)
    account_id = prepare_account_with_route(account_store, identity)
    now = datetime(2026, 6, 15, 12, tzinfo=timezone.utc)
    ...
    too_early = queue_due_notification_loudness_prompts(account_store, account_id, now=now + timedelta(hours=11, minutes=59))
    due = queue_due_notification_loudness_prompts(account_store, account_id, now=now + timedelta(hours=12))
    duplicate = queue_due_notification_loudness_prompts(account_store, account_id, now=now + timedelta(hours=12, minutes=1))
...
def test_notification_loudness_system_item_dispatches_without_proactive_consent(tmp_path) -> None:
    ...
    account_id = prepare_account_with_route(account_store, identity)
    now = datetime(2026, 6, 15, 12, tzinfo=timezone.utc)
    assert maybe_notification_loudness_prompt_action(event(identity), account_store, account_id, now=now - timedelta(hours=12)) is not None
